...
async function connect(config) {
  ...
  var pool = mysql.createPool({
    ...
    // A VALUE IS A VALUE. Without this mysql2 expands an object bound to `?`
    // into `key` = value pairs and nested arrays into lists — so a filter value
    // of {"a":1} rewrites the predicate instead of being compared to it.
    stringifyObjects:  true
    // multipleStatements stays at its default (off): one statement per query.
  });
  // READ-ONLY CONNECTIONS — `connect({ ..., readOnly: true })` for SQL the
  // caller did not write itself. Every connection the pool opens is put into
  // read-only transaction mode before it is used, so a write fails.
  if (config.readOnly) {
