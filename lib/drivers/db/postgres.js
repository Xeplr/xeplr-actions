...
// READ-ONLY CONNECTIONS — for SQL the caller did not write itself (a report
// preview, a user's query). `connect({ ..., readOnly: true })` makes every
// query and fetchStream on that pool:
//
//   • run inside BEGIN TRANSACTION READ ONLY, always rolled back — a write
//     fails with "cannot execute … in a read-only transaction", and nothing
//     inside the transaction can switch it back to read-write
//   • go over the EXTENDED protocol, which carries exactly one statement — so
//     `…; COMMIT; DROP TABLE t` is refused rather than run after the SELECT
//     (the simple protocol, which pg uses when there are no parameters, runs
...
async function connect(config) {
  ...
  if (config.readOnly) pool.__xeplrReadOnly = true;
  return pool;
}
...
// `columns` is the query's OUTPUT COLUMNS, which the server describes whether
// or not any row matched. Callers that need the shape of a result — a report
// listing its columns, say — must not have to infer it from the first row,
// because a query returning nothing still has columns.
//
// Attached to pg's own Result rather than wrapping it: callers already treat
// this as the canonical shape the other drivers normalize TO, so replacing
// the object risks dropping a property something else reads.
async function query(pool, sql, params) {
  var result;
  if (pool.__xeplrReadOnly) {
    var client = await pool.connect();
    try {
      await client.query('BEGIN TRANSACTION READ ONLY');
      result = await client.query({ text: sql, values: params || [], queryMode: 'extended' });
    } finally {
      try { await client.query('ROLLBACK'); } catch (_) { /* the connection is released either way */ }
      client.release();
    }
  } else {
    result = await pool.query(sql, params || []);
  }
  result.columns = (result.fields || []).map(function(f) { return f.name; });
  return result;
}
...
async function* fetchStream(pool, opts) {
  ...
  var declared = false;
  try {
    var readOnly = Boolean(pool.__xeplrReadOnly);
    await client.query(readOnly ? 'BEGIN TRANSACTION READ ONLY' : 'BEGIN');
    // Extended protocol when read-only: one statement, so the caller's SQL
    // cannot close the cursor declaration and carry on with its own.
    await client.query(readOnly
      ? { text: 'DECLARE __xeplr_fetch_cur NO SCROLL CURSOR FOR ' + opts.sql, values: opts.params || [], queryMode: 'extended' }
      : 'DECLARE __xeplr_fetch_cur NO SCROLL CURSOR FOR ' + opts.sql, readOnly ? undefined : (opts.params || []));
    declared = true;
    while (true) {
    ...
    try { await client.query(pool.__xeplrReadOnly ? 'ROLLBACK' : 'COMMIT'); } catch (_) {}
    client.release();
  }
}
