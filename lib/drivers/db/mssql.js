...
var NATIVE_TO_LOGICAL = {
  ...
  // 'date' maps to its OWN logical type — see reconcile.js.
  date: 'date', time: 'datetime', datetimeoffset: 'datetime'
};
...
async function connect(config) {
  ...
  var pool = new sql.ConnectionPool({
    ...
    options:  Object.assign(
      ...
      // readOnly: ApplicationIntent=ReadOnly — routed to a readable secondary
      // where there is one. It is NOT enforcement: SQL Server runs a whole
      // batch and has no session-level read-only mode, so SQL the caller did
      // not write needs a login that can only read (db_datareader). See README.
      config.readOnly ? { readOnlyIntent: true } : {},
      config.options || {}
    )
  });
  await pool.connect();
...
async function query(pool, sql, params) {
  ...
  var rowCount = Array.isArray(r.rowsAffected)
    ...
    : rows.length;
  // recordset.columns is present on an EMPTY recordset too — mssql describes
  // the result's shape independently of how many rows matched, which is
  // exactly what a caller listing a query's columns needs. It's keyed by name
  // with an `index`, so sort by that to keep the SELECT's own order.
  var meta = (r.recordset && r.recordset.columns) || {};
  var columns = Object.keys(meta).sort(function(a, b) { return meta[a].index - meta[b].index; });
  return { rows: rows, rowCount: rowCount, columns: columns };
}
