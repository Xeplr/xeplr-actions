...
function buildProcedureCall(opts) {
  ...
  var marks = entries.map(function(p, i) {
    ...
    // Written into the call as text (`@region = @p0`, `region => $1`), so a
    // plain name only — a saved or requested parameter name must not be able
    // to close the call and start another statement.
    if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(String(p.name))) {
      throw new Error('procedure parameter name "' + String(p.name).slice(0, 40) + '" — use letters, digits and _ only');
    }
