...
//     columns,         // optional — [{ name, type }] in the logical vocabulary
//                       // (string|number|boolean|date|datetime|object|array).
//                       // Supplied, types are DECLARED and the first batch is
//                       // not sampled at all. Omitted, they are inferred from
//                       // the first batch's values, which is right for a CSV
//                       // and wrong wherever the source has a schema to ask.
//                       // See beforeAll.
...
//     onProgress,      // optional ({rowsRead, batches}) => void, called per batch.
//                       // Rows READ, not rows confirmed written — see onBatch.
//                       // Throwing in it cannot fail the movement.
//   })
//
...
async function upload(opts) {
  ...
  try {
    var summary = await spool(opts.source, {
      ...
      target: {
        // Bootstrap: first batch is on disk. Work out the column types, run
        // DDL, reconcile against the target's existing schema, then compute
        // the safe batch size for INSERT.
        beforeAll: async function(ctx) {
          runDir = ctx.batchDir;
          var declared;
          if (opts.columns && opts.columns.length) {
            // DECLARED, so nothing is guessed.
            //
            // Inference reads VALUES, which is the only option for a CSV and
            // the wrong one when the source has a schema you can just ask for.
            // The failures are quiet and permanent: Postgres hands `numeric`
            // to node as a STRING, so a money column arrives as strings, is
            // inferred as text, and the target column is created TEXT — you
            // find out when somebody tries to SUM it. An all-null sample types
            // as string for the same reason. A zero-padded code becomes a
            // number and loses its leading zeros.
            //
            // The guess is fine when it is all you have. It should not be what
            // happens when it is not.
            declared = opts.columns;
          } else {
            var sample = await readBatchFile(ctx.firstBatchPath);
            var scanLimit = Math.min(sample.length, opts.firstBatchScanRows);
            declared = inferColumns(sample.slice(0, scanLimit));
          }
          columns = await runBootstrap(driver, pool, opts.targetTable, declared, opts.primaryKeys);
        ...
        onBatch: async function(rows, meta) {
          ...
          for (var off = 0; off < rows.length; off += effectiveBatchSize) {
            ...
            // A NAME FOR THIS BATCH, carried all the way to the failure.
            //
            // Without one, "20 rows dropped" is unactionable: there is nothing
            // to look up, nothing to re-run, nothing to tell somebody over the
            // phone. b<spool batch>.<chunk> is stable for a given run and
            // enough for a human to find the rows again in the source.
            var batchRef = 'b' + (meta ? meta.batchIndex : totalBatches) +
              '.' + Math.floor(off / effectiveBatchSize);
            await enqueueBatch(opts, chunk, columns, batchRef);
            totalBatches++;
          }
          rowsSeen += rows.length;
          // WHAT HAS BEEN READ, not what has landed — the queue is still
          // draining behind this. Deliberately so: for a long movement the only
          // honest live number is "rows so far", and waiting for the write to
          // confirm would report nothing for minutes.
          //
          // No total, because for a query source there isn't one to know
          // without running it twice. A caller that wants a percentage has to
          // supply its own denominator.
          if (opts.onProgress) {
            try {
              opts.onProgress({ rowsRead: rowsSeen, batches: meta ? meta.batchIndex + 1 : totalBatches });
            } catch (e) { /* a reporting failure must not fail the movement */ }
          }
        }
      }
    });
...
//
// Refuses outright for any movement recorded with primaryKeys (i.e. it ran
// as an upsert). UPSERT's ON CONFLICT/ON DUPLICATE KEY/MERGE stamps
// __xeplr_movement_id__ onto every row it MATCHES, not just the ones it
// creates (see buildInsertSql in each driver) — so a pre-existing row that
// was merely updated ends up tagged with this run's movementId too. Deleting
// by that column can't tell "created by this run" from "already existed and
// got touched by it" apart after the fact, so there is no safe partial
// rollback to fall back to — the whole operation is refused.
async function rollback(opts) {
  ...
  if (opts.metaStore && opts.metaStore.getById) {
    var meta = await opts.metaStore.getById(opts.movementId);
    if (meta && meta.primary_keys && meta.primary_keys.length) {
      throw new Error(
        'rollback: this movement used upsert (primary keys: ' + meta.primary_keys.join(', ') + ') — ' +
        'rolling back could delete rows that already existed and were only updated, not created. ' +
        'Rollback is not supported for upsert movements.'
      );
    }
  }
...
async function enqueueBatch(opts, rows, columns, batchRef) {
  var driver     = opts.driver;
  ...
  await opts.queue.addToQueue({
    ...
    meta: {
      ...
      // Travels with the statement so a drop can name which batch it was.
